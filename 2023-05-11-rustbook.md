...
cf. [mes notes de workflow](./structured/workflow/dev-workflow.md).
...
3 trucs discutés dans ce chapitre :
...
En plus de protéger contre les erreurs de paramètres et de rendre l'intention du code plus explicite, les newtypes peuvent également servir à encapsuler un type afin de masquer le type sous-jacent (qui devient un détail d'implémentation, l'API publique étant celle qu'on choisit d'exposer sur le newtype).

**Creating Type Synonyms with Type Aliases**

Rien de nouveau : type alias pour éviter d'écrire des types à rallonge + pour mieux exprimer l'intention (et éventuellement pour renommer des types et éviter des confusions).

**The Never Type that Never Returns**

Il existe un type spécial qui indique qu'une fonction ne retourne jamais : `!`

D'après les exemples, il aide surtout à respecter la contrainte imposant que les deux arms d'un bloc match renvoie une valeur de même type.

**Dynamically Sized Types and the Sized Trait**

La plupart des types manipulés en rust ont une taille (= empreinte mémoire) connue à la compilation. Ça n'est pas le cas pour les DST = dynamically sized types.
...
^ du coup, pour les manipuler, on passe par un couple (pointeur + taille), qui, lui, a un type connu statiquement ; le pointeur peut être une référence ou un smart-pointer.

Les exemples sont donnés avec `str`, qui est un DST qu'on manipule avec `&str` (qui associe un pointeur vers le `str` et sa taille).
...
^ le trait `Sized` est automatiquement implémenté par tous les types dont la taille est connue.

À noter que par défaut, les fonctions génériques ont une contrainte implicite sur le type générique = il doit être `Sized` ; mais il existe une syntaxe spéciale pour qu'une fonction générique accepte un paramètre non `Sized` :
...
^ rappel : les DST se manipulent obligatoirement avec des formes de pointeurs.
...
Les pointeurs de fonctions sont différents des callables ; une fonction peut attendre l'un ou l'autre comme argument :
...
// avec un type qui implémente Fn = un callable :
...
Quelque chose qui implémente `Fn` (ou ses dérivés) véhicule la notion générale de "callable" (y compris p.ex. des closures qui capture leur environnement), alors qu'un `fn` est le type d'une fonction.

Une closure peut éventuellement être une `fn` à condition qu'elle ne capture rien de son environnement.
...
^ partout où on peut passer un callable, on peut passer une fn, donc mieux vaut qu'une fonction qui prend une autre fonction en argument la prenne sous la forme d'un callable (i.e. un type générique qui implémente le trait `Fn`).

Une différence notable : pour qu'une fonction f accepte un callable, f doit être générique sur un type générique implémentant `Fn`, alors que pour accepter une fonction, f n'a pas besoin d'être générique.

Une situation fréquente où on passe des pointeurs de fonctions : quand on veut passer à la fonction acceptant un callable (telle que `map`) un constructeur de variants d'enum.


Quelques liens avec des éclaircissements : [lien1](https://stackoverflow.com/questions/54181816/function-pointer-vs-fn-trait-object), [lien2](https://users.rust-lang.org/t/fn-vs-fn-in-type-signatures/26108).

Par ailleurs, la doc de référence [sur le trait Fn](https://doc.rust-lang.org/std/ops/trait.Fn.html) et [sur les pointeurs de fonctions avec fn](https://doc.rust-lang.org/std/primitive.fn.html)


**Returning Closures**

On ne peut pas faire une fonction qui retourne une closure. À la place, il faut retourner un trait object :
...
> Fundamentally, macros are a way of writing code that writes other code, which is known as metaprogramming (...) All of these macros expand to produce more code than the code you’ve written manually.

^ preprocessor like

Pourquoi utiliser des macros plutôt que des fonctions ?

- à la différence des fonctions, les macros peuvent avoir un nombre variable de paramètres (e.g. `vec!`)
- les macros s'exécutent avant le compilo, donc à la différence des fonctions, elles peuvent produire des résultats dont on aurait besoin au compile-time (exemple : elles peuvent implémenter des traits, comme le fait `derive`)

Pourquoi ne PAS utiliser les macros ? Elles sont plus dures à lire, comprendre et maintenir.

Il y a deux types de macros :

- **declarative macros** aka "macros by example" aka "macro_rules! macros"
- **procedural macros**, qui se déclinent à leur tour en :
    - **derive macros**
...
En gros, avec une macro déclarative, son corps est un gros bloc match qui matche les paramètres passés à la macro avec des patterns, et qui produit le code de l'arm correspondant :
...
Le rustbook recommande un book sur les macros déclaratives : https://veykril.github.io/tlborm/
...
L'idée de base des macros procédurales, c'est de convertir un code (= `TokenStream`) donné en entrée en un code modifié en sortie :
...
L'exemple montre pas à pas comment implémenter une macro pour dériver un trait :
...
- on déclare une macro derive avec `proc_macro_derive`
- on parse le TokenStream en input avec la carte `syn`
- on écrit un template de code à remplir avec des infos issues du TokenStream d'input
- on convertit le template de code rempli en TokenStream de sortie avec la crate `quote`

**Attribute-like macros**

Les attributs ressemblent à `derive` ce sont des annotations qu'on ajoute à des items pour leur ajouter des fonctionnalités :
...
`derive` ne peut être appliquée que sur struct et enum, les attributs peuvent être appliqués à plus de trucs, comme les fonctions.

Les attributes macros fonctionnent comme pour `derive`, sauf qu'on utilise plutôt `proc_macro_attribute`

**Function-like macros**

Les fonctions-like macros ressemblent aux `macro_rules!` macros : on les appelle comme des fonctions.

Mais là où les macro_rules sont obligatoirement codées sous la forme d'un bloc `match`, les functions-like macros n'ont pas cette contrainte sur leur implementation : elles peuvent faire du procédural.

On les déclare avec `proc_macro`.

## Chapitre 20 = Final Project: Building a Multithreaded Web Server

https://doc.rust-lang.org/book/ch20-00-final-project-a-web-server.html

Ce chapitre est un projet d'application, à visée éducationnelle :
...
Rien d'extraordinaire dans la première partie : on binde une socket TCP à l'adresse locale + port, et on renvoie la réponse HTTP appropriée.

La seconde partie est un peu plus intéressante, puisqu'il s'agit de coder un threadpool pour répondre aux requêtes.

Le threadpool utilise un channel pour transmettre des closures (des `Box<F>` où `F` est la closure à exécuter) à des threads afin que ces derniers les exécutent. Les threads pullent en boucle sur le receiver du channel (partagé avec un `Arc<Mutex<T>>`) et exécutent chaque closure reçue.

Un conseil intéressant en passant :
...
La troisième partie est également intéressante : il s'agit de ne pas quitter abruptement quand on arrête le serveur, mais d'indiquer aux threads de finir de traiter leur requête en cours sans en accepter de nouvelles.

Pour celà, le threadpool implémente `Drop`,  avec la logique qui va bien : il droppe le sender du channel, ce qui permet de communiquer aux thread qui pullent qu'ils doivent s'arrêter. Chaque thread s'arrêtera au prochain pull, ce qui lui laisse le temps de finir de traiter sa requête en cours, comme souhaité.
...
Liste des keywords réservés par le langage (y compris ceux qui sont encore inutilisés) + comment nommer des variables comme les keywords avec `r#keyword`

### 21.2. B - Operators and Symbols

https://doc.rust-lang.org/book/appendix-02-operators.html

Liste des operators, et des traits permettant de les overloader pour ceux qui sont concernés.
...
^ `derive` sert à générer une implémentation par défaut à un de ses types (ça évite d'avoir à écrire plein de boilerplate).

Liste de tous les traits dérivables dans la stdlib (mais d'autres librairies peuvent étendre cette liste) :
...
^ différences entre `Eq` et `PartialEq`. Un cas où la différence est importante : pour être là clé d'une HashMap, on a besoin d'être sûr qu'une valeur passée est égale à la clé stockée.

Ma compréhension, c'est que PartialEq permet de dire que deux valeurs sont différentes, mais ne permet pas de dire à coup sûr qu'elles sont égales. Eq permet de dire à coup sûr que deux valeurs sont égales.

Différence entre PartialOrd et Ord : avec PartialOrd, certaines valeurs du type sont ordonnées (i.e. on peut dire que l'une est inférieure à l'autre) mais d'autres non : un exemple est ici aussi float, puisque NaN n'est pas ordonné. Si un type implémente Ord, TOUTES les valeurs possibles ont une relation d'ordre.
...
Une release de rust est livrée toutes les quelques semaines.

Toutes les quelques années, les releases livrées jusqu'ici sont packagées dans une **édition**, à date, il en existe trois :

- 2015
- 2018
- 2021

Même si chaque nouvelle édition apporte son lot de nouveautés, elles restent rétro et forward compatibles : les librairies compilées avec une édition sont utilisables dans un binaire qui utilise une autre édition.

### 21.6. F - Translations of the Book

https://doc.rust-lang.org/book/appendix-06-translation.html

RAS, le book est traduit.

### 21.7. G - How Rust is Made and “Nightly Rust”

https://doc.rust-lang.org/book/appendix-07-nightly-rust.html

Cette page explique la façon dont rust est livré, avec les trois release channels :

- Nightly
- Beta
- Stable

rustup permet d'installer une autre version que la version stable, y compris juste pour un projet.

L'équipe rust fonctionne en plusieurs teams, chacune travaillent sur un morceau, qui renvoient les RFC qui leur sont soumises, cf. https://www.rust-lang.org/governance
